...
PG_FUNCTION_INFO_V1(ecdk_sync_data);
Datum ecdk_sync_data(PG_FUNCTION_ARGS)
{
    ...
    while(true)
    {
        ...
        for(size_t i = 0; i < SPI_processed; i++)
        {
            ...
            if(ARR_NDIM(fpArray))
            {
                ...
                for(int j = bitset_next_set_bit(&fp, 0); j >= 0 && j < FP_SIZE; j = bitset_next_set_bit(&fp, j + 1))
                    bitset_set(bitmap + j, seqid);
            }
        }
    ...
    EcdkLoaderData *data = palloc(SYNC_FETCH_SIZE * sizeof(EcdkLoaderData));
    int currentSeqid = -1;
    int notIndexed = 0;
    int count = 0;
    ...
    while(true)
    {
        ...
        ecdk_java_parse_data(processed, molfiles, data);
        ...
        for(int i = 0; i < processed; i++)
        {
            ...
            for(int j = bitset_next_set_bit(&fp, 0); j >= 0 && j <= FP_SIZE; j = bitset_next_set_bit(&fp, j + 1))
                bitset_set(bitmap + j, currentSeqid);
    ...
    char *indexFilePath = get_index_path(ECDK_INDEX_PREFIX, ECDK_INDEX_SUFFIX, indexNumber);
...
PG_FUNCTION_INFO_V1(ecdk_cleanup);
Datum ecdk_cleanup(PG_FUNCTION_ARGS)
{
    if(unlikely(SPI_connect() != SPI_OK_CONNECT))
        elog(ERROR, "%s: SPI_connect() failed", __func__);
    ...
    char *ecdkIndexName = get_index_name(ECDK_INDEX_PREFIX, ECDK_INDEX_SUFFIX, indexNumber);
#if USE_MOLECULE_INDEX
    ...
    PG_TRY();
    {
        ...
        while((ep = readdir(dp)))
        {
            if(!strncmp(ep->d_name, ECDK_INDEX_PREFIX, sizeof(ECDK_INDEX_PREFIX) - 1))
            {
                if(!strcmp(ep->d_name, ecdkIndexName))
                    continue;

                elog(NOTICE, "delete ecdk index '%s'", ep->d_name);
