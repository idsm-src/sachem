...
typedef struct
{
    ...
    EcdkSubstructureQueryData *queryData;
    int queryDataCount;
...
static void ecdk_subsearch_init(void)
{
    ...
    if(unlikely(dbIndexNumber != indexId))
    {
        ...
        PG_TRY();
        {
            ...
            char *indexFilePath = get_index_path(ECDK_INDEX_PREFIX, ECDK_INDEX_SUFFIX, dbIndexNumber);
...
PG_FUNCTION_INFO_V1(ecdk_substructure_search);
Datum ecdk_substructure_search(PG_FUNCTION_ARGS)
{
    if(SRF_IS_FIRSTCALL())
    {
#if SHOW_STATS
...
#endif
        info->queryDataCount = ecdk_java_parse_substructure_query(&info->queryData, VARDATA(query), VARSIZE(query) - VARHDRSZ,
        ...
        while(true)
        {
            if(unlikely(info->tableRowPosition == info->tableRowCount))
            {
                ...
                if(info->candidatePosition < 0)
                {
                    ...
                    EcdkSubstructureQueryData *data = &(info->queryData[info->queryDataPosition]);
