...
PG_FUNCTION_INFO_V1(lucy_cleanup);
Datum lucy_cleanup(PG_FUNCTION_ARGS)
{
    if(unlikely(SPI_connect() != SPI_OK_CONNECT))
        elog(ERROR, "%s: SPI_connect() failed", __func__);
    ...
    char *lucyIndexName = get_index_name(LUCY_INDEX_PREFIX, LUCY_INDEX_SUFFIX, indexNumber);
#if USE_MOLECULE_INDEX
    ...
    PG_TRY();
    {
        ...
        while((ep = readdir(dp)))
        {
            if(!strncmp(ep->d_name, LUCY_INDEX_PREFIX, sizeof(LUCY_INDEX_PREFIX) - 1))
            {
                if(!strcmp(ep->d_name, lucyIndexName))
                    continue;

                elog(NOTICE, "delete lucy index '%s'", ep->d_name);

                char *lucyIndexPath = get_file_path(ep->d_name);
                lucy_delete_directory(lucyIndexPath);
            }
#if USE_MOLECULE_INDEX
