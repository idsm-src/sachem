...
PG_FUNCTION_INFO_V1(orchem_cleanup);
Datum orchem_cleanup(PG_FUNCTION_ARGS)
{
    if(unlikely(SPI_connect() != SPI_OK_CONNECT))
        elog(ERROR, "%s: SPI_connect() failed", __func__);
    ...
    char *orchemIndexName = get_index_name(ORCHEM_INDEX_PREFIX, ORCHEM_INDEX_SUFFIX, indexNumber);
#if USE_MOLECULE_INDEX
    ...
    PG_TRY();
    {
        ...
        while((ep = readdir(dp)))
        {
            if(!strncmp(ep->d_name, ORCHEM_INDEX_PREFIX, sizeof(ORCHEM_INDEX_PREFIX) - 1))
            {
                if(!strcmp(ep->d_name, orchemIndexName))
                    continue;

                elog(NOTICE, "delete orchem index '%s'", ep->d_name);
