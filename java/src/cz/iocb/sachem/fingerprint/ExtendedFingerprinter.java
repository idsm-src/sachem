...
/**
 * Generates an extended fingerprint for a given {@link IAtomContainer}, that extends the {@link Fingerprinter} with
 * additional bits describing ring features.
 *
 * @author shk3
 ...
 */
public class ExtendedFingerprinter extends AbstractFingerprinter implements IFingerprinter
{
    ...
    /**
     * Creates a fingerprint generator of length <code>DEFAULT_SIZE</code> and with a search depth of
    ...
    public ExtendedFingerprinter(int size, int searchDepth)
    {
        this.fingerprinter = new Fingerprinter(size - RESERVED_BITS, searchDepth)
        {
                    ...
                    throws CDKException
            {
                ...
                encodePaths(container, getSearchDepth(), bitSet, getSize());
    ...
    /**
     * Generates a fingerprint of the default size for the given AtomContainer, using path and ring metrics. It contains
     * the informations from getBitFingerprint() and bits which tell if the structure has 0 rings, 1 or less rings, 2 or
     * less rings ... 10 or less rings (referring to smallest set of smallest rings) and bits which tell if there is a
     * fused ring system with 1,2...8 or more rings in it
     *
     ...
     * Generates a fingerprint of the default size for the given AtomContainer, using path and ring metrics. It contains
     * the informations from getBitFingerprint() and bits which tell if the structure has 0 rings, 1 or less rings, 2 or
     * less rings ... 10 or less rings and bits which tell if there is a fused ring system with 1,2...8 or more rings in
     * it. The RingSet used is passed via rs parameter. This must be a smallesSetOfSmallestRings. The List must be a
     * list of all ring systems in the molecule.
     *
     * @param atomContainer The AtomContainer for which a Fingerprint is generated
     * @param ringSet An SSSR RingSet of ac (if not available, use getExtendedFingerprint(AtomContainer ac), which does
     *            the calculation)
            ...
            throws CDKException
    {
        ...
        double weight = MolecularFormulaManipulator
                .getTotalNaturalAbundance(MolecularFormulaManipulator.getMolecularFormula(container));
        ...
        {
            if(rslist.get(i).getAtomContainerCount() > maximumringsystemsize)
